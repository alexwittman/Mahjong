...
class _Yaku_Evaluate {
    MeldsOfType(partition : (_Meld | _Pair)[], type : _MeldType) : _Meld[] {
        let melds : _Meld[] = []
    ...
    PartitionTiles(partition : (_Meld | _Pair)[]) : _Tile[] {
        let tiles : _Tile[] = [];
        for(let meld of partition){
    ...
    EvaluateYaku(partition : (_Meld | _Pair)[]) : _Yaku[] {
        let yakuList : _Yaku[] = [];
    ...
    NoPointsHand(partition : (_Meld | _Pair)[]) : _Yaku {
        let chows : _Meld[] = this.MeldsOfType(partition, _MeldType.CHOW);
        if(chows.length == 4){
    ...
    IdenticalSequences(partition : (_Meld | _Pair)[]) : _Yaku {
        let chows : _Meld[] = this.MeldsOfType(partition, _MeldType.CHOW);
        if(chows.length < 2) return null;
        for(let i : number = 0; i < chows.length - 1; i++){
            for(let j : number = i + 1; j < chows.length; j++){
    ...
    ThreeColorStraight(partition : (_Meld | _Pair)[]) : _Yaku {
        let chows : _Meld[] = this.MeldsOfType(partition, _MeldType.CHOW);
    ...
    Straight(partition : (_Meld | _Pair)[]) : _Yaku {
        let chows : _Meld[] = this.MeldsOfType(partition, _MeldType.CHOW);
        if(chows.length < 3) return null;
        if(chows.length == 3){
    ...
    DoubleIdenticalSequences(partition : (_Meld | _Pair)[]) : _Yaku {
        let chows : _Meld[] = this.MeldsOfType(partition, _MeldType.CHOW);
    ...
    AllTripletHand(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        if(pongs.length == 4){
    ...
    ThreeClosedTriplets(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        if(pongs.length < 3) return null;
        let closedPongCount : number = 0;
    ...
    ThreeColorTriplets(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        if(pongs.length < 3) return null;
        if(pongs.length == 3){
    ...
    ThreeQuads(partition : (_Meld | _Pair)[]) : _Yaku {
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        if(kongs.length == 3){
    ...
    AllSimples(partition : (_Meld | _Pair)[]) : _Yaku {
        let NonSimples : number[] = TERMINALS.concat(HONORS);
        let tiles : _Tile[] = this.PartitionTiles(partition);
    ...
    RedDragon(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        for(let pong of pongs){
    ...
    GreenDragon(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        for(let pong of pongs){
    ...
    WhiteDragon(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        for(let pong of pongs){
    ...
    NorthWind(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
    ...
    EastWind(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        for(let meld of melds){
    ...
    SouthWind(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        for(let meld of melds){
    ...
    WestWind(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        for(let meld of melds){
    ...
    MixedOutsideHand(partition : (_Meld | _Pair)[]) : _Yaku {
        let OutsideTiles : number[] = HONORS.concat(TERMINALS);
        let PartitionOutsideTiles : boolean[] = [];
        for(let meld of partition){
            let MeldOutsideTiles : boolean[] = [];
            for(let tile of meld.tiles){
                let OutsideTilesIndexes : number[] = [];
    ...
    PureOutsideHand(partition : (_Meld | _Pair)[]) : _Yaku {
        let OutsideTiles : number[] = HONORS.concat(TERMINALS);
        let PartitionOutsideTiles : boolean[] = [];
        for(let meld of partition){
            let MeldOutsideTiles : boolean[] = [];
    ...
    LittleThreeDragons(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        ...
        if(!(partition[partition.length - 1].tiles[0].value == _TileValue.GREEN ||
             ...
             partition[partition.length - 1].tiles[0].value == _TileValue.WHITE)) return null;
        let DragonValues : _TileValue[] = [_TileValue.GREEN, _TileValue.RED, _TileValue.WHITE];
    ...
    HalfFlush(partition : (_Meld | _Pair)[]) : _Yaku {
        let tiles : _Tile[] = this.PartitionTiles(partition);
        let tileTypes : _TileType[] = [];
        for(let tile of tiles){
        ...
        let type : _TileType = tileTypes[0];
    ...
    Flush(partition : (_Meld | _Pair)[]) : _Yaku {
        let tiles : _Tile[] = this.PartitionTiles(partition);
        let tileTypes : _TileType[] = [];
        for(let tile of tiles){
            tileTypes.push(tile.type);
    ...
    ThirteenOrphans(hand : _Hand, winningTile : _Tile) : _Yaku {
        let tiles : _Tile[] = CopyTileList(hand.tiles);
    ...
    DoubleThirteenOrphans(hand : _Hand, winningTile : _Tile) : _Yaku {
        let orphans = [ new _Tile(PIN_ONE), new _Tile(PIN_NINE), new _Tile(SOU_ONE), new _Tile(SOU_NINE),
                        new _Tile(WAN_ONE), new _Tile(WAN_NINE), new _Tile(EAST), new _Tile(SOUTH),
                        new _Tile(WEST), new _Tile(NORTH), new _Tile(DRAGON_GREEN), new _Tile(DRAGON_RED),
                        new _Tile(DRAGON_WHITE)];
    ...
    FourConcealedTriplets(partition : (_Meld | _Pair)[], hand : _Hand, winningTile : _Tile) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        ...
        let tiles : _Tile[] = CopyTileList(hand.tiles);
        let tilesWithoutWinningTile = RemoveFromTileList(tiles, winningTile);
        let tenpai : _Tenpai = new _Tenpai(new _Hand(tilesWithoutWinningTile));
    ...
    DoubleFourConcealedTriplets(partition : (_Meld | _Pair)[], hand : _Hand, winningTile : _Tile) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        if(melds.length < 4) return null;
    ...
    BigThreeDragons(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
    ...
    LittleFourWinds(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        if(melds.length < 3) return null;
        if(!(partition[partition.length - 1].tiles[0].value == _TileValue.NORTH ||
             ...
             partition[partition.length - 1].tiles[0].value == _TileValue.WEST)) return null;
        let WindValues : _TileValue[] = [_TileValue.NORTH, _TileValue.EAST, _TileValue.SOUTH, _TileValue.WEST];
    ...
    BigFourWinds(partition : (_Meld | _Pair)[]) : _Yaku {
        let pongs : _Meld[] = this.MeldsOfType(partition, _MeldType.PONG);
        let kongs : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        let melds : _Meld[] = pongs.concat(kongs);
        if(melds.length < 4) return null;
    ...
    AllHonors(partition : (_Meld | _Pair)[]) : _Yaku {
        let Honors : number[] = HONORS;
        let tiles : _Tile[] = this.PartitionTiles(partition);
        for(let tile of tiles){
    ...
    AllTerminals(partition : (_Meld | _Pair)[]) : _Yaku {
        let Terminals : number[] = TERMINALS;
        let tiles : _Tile[] = this.PartitionTiles(partition);
        for(let tile of tiles){
    ...
    AllGreen(partition : (_Meld | _Pair)[]) : _Yaku {
    ...
    NineGates(hand : _Hand, winningTile : _Tile) : _Yaku {
        let tiles : _Tile[] = CopyTileList(hand.tiles);
        let tilesWithoutWinningTile : _Tile[] = RemoveFromTileList(tiles, winningTile);
        let tenpai : _Tenpai = new _Tenpai(new _Hand(tilesWithoutWinningTile));
        for(let tile of tiles){
            let tempTiles : _Tile[] = CopyTileList(hand.tiles);
            let tilesWithoutTile : _Tile[] = RemoveFromTileList(tempTiles, tile);
            let tenpai : _Tenpai = new _Tenpai(new _Hand(tilesWithoutTile));
    ...
    DoubleNineGates(hand : _Hand, winningTile : _Tile) : _Yaku {
        let tiles : _Tile[] = CopyTileList(hand.tiles);
        let tilesWithoutWinningTile : _Tile[] = RemoveFromTileList(tiles, winningTile);
        let tenpai : _Tenpai = new _Tenpai(new _Hand(tilesWithoutWinningTile));
    ...
    FourQuads(partition : (_Meld | _Pair)[]) : _Yaku {
        let quads : _Meld[] = this.MeldsOfType(partition, _MeldType.KONG);
        if(quads.length == 4){
